...
//	Declare a public Transform called pathmakerSpherePrefab, assign the prefab in inspector; 		// you'll have to make a "pathmakerSphere" prefab later
	...
	void Start(){
		...
		originalProb=turnProbability;
		originalCorSize=corridorSize;
...
//			Destroy my game object; 		// self destruct if I've made enough tiles already
			...
			if(floorAdded){
				...
				if(roomMode && counter>roomSize){
					...
					roomSize=Random.Range(originalRoomSize-10,originalRoomSize+20);
					counter=0;
					roomMode=!roomMode;
				}else if(!roomMode && counter>corridorSize){
					turnProbability=originalProb;
					corridorSize=Random.Range(originalCorSize-3,originalCorSize+3);
					counter=0;
					roomMode=!roomMode;
				}
			}else{
